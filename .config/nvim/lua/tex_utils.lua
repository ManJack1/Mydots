...
-- 获取当前光标所在节点的工具函数
function tex_utils.get_node_at_cursor()
...
-- 获取节点文本的工具函数
function tex_utils.get_node_text(node)
  if not node then
    return ""
  end
...
local function in_specific_env(envs)
  local node = tex_utils.get_node_at_cursor()
  while node do
    if node:type() == "generic_environment" then
      local start_tag = node:child(0)
      if start_tag and start_tag:type() == "begin" then
        local text = tex_utils.get_node_text(start_tag)
...
function tex_utils.in_bib()
  return tex_utils.in_env("thebibliography")
end

function tex_utils.in_minipage()
  return tex_utils.in_env("minipage")
end

function tex_utils.in_frame()
  return tex_utils.in_env("frame")
end

function tex_utils.in_ampersand_env()
